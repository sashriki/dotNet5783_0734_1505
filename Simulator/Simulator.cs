    ...
    private static void simulationProgress()
    {
        ...
        while (!s_flagStopSimulation)
        {
            ...
            else
            {
                ...
                order = order.OrderStatus == OrderStatus.Confirmed ? _bl.Order.ShippingUpdateToManager(order.OrderId)
                        : _bl.Order.supplyUpdateToManager(order.OrderId);

                items =
                     Tuple.Create(order, (order.OrderStatus == OrderStatus.Shipped ? (OrderStatus?)OrderStatus.Delivered : null), "", "");
